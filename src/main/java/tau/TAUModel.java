...
public class TAUModel extends VIVIDCoreModel<Globals> {
  ...
  public void init() {
    ...
    getGlobals().overallExternalInfectionRateFromData =
        externalData.getExternalDataInfectionRate(getGlobals().stateFile,
            getGlobals().externalDataCounty,
            getGlobals().externalDataState,
            getGlobals().externalDataSource);
    ...
    public double getExternalInfectionRate(Person person, Globals globals) {
      ...
      if(person instanceof Student) {
        if (((Student) person).livesAtBuilding == null) {
          sum += globals.baseOffCampusExternalInfectionRate;
        } else {
          sum += globals.baseOnCampusExternalInfectionRate;
        }
      }

      if (person.usesPublicTransit) {
        sum += globals.marginalPublicTransitExternalInfectionRate;
      }
      return sum;
    }
