...
public class StaticNetworkBuilderTest {
  ...
  @Test
  public void testClassSchedules_hybrid() {
    testKit.getGlobals().hybridClassesEnabled = true;
    ...
    for (PlaceInfo p : mwfClasses) {
      ...
      assertThat(Sets.difference(mondayers, ImmutableSet.of(p.center())))
          .containsNoneIn(Sets.difference(wednesdayers, ImmutableSet.of(p.center())));

      assertThat(Sets.difference(mondayers, ImmutableSet.of(p.center())))
          .containsNoneIn(Sets.difference(fridayers, ImmutableSet.of(p.center())));

      assertThat(Sets.difference(wednesdayers, ImmutableSet.of(p.center())))
          .containsNoneIn(Sets.difference(fridayers, ImmutableSet.of(p.center())));
    }
    ...
    for (PlaceInfo p : tuthClasses) {
      ...
      // Check there is no student overlap
      assertThat(Sets.difference(tuesdayers, ImmutableSet.of(p.center())))
          .containsNoneIn(Sets.difference(thursdayers, ImmutableSet.of(p.center())));
    }
    ...
    for (PlaceInfo p : oneDayClasses) {
      ...
      // Check there is no student overlap
      assertThat(Sets.difference(firstWeekers, ImmutableSet.of(p.center())))
          .containsNoneIn(Sets.difference(secondWeekers, ImmutableSet.of(p.center())));
    }
  }

  @Test
  public void testClassSchedules_noHybrid() {
    testKit.getGlobals().hybridClassesEnabled = false;
    StaticNetworkBuilder staticNetworkBuilder = new StaticNetworkBuilder();
    staticNetworkBuilder.createConnectionOfAgents(allPeople, testKit.getGlobals());
    Map<Long, Person.DailySchedule> schedules =
        staticNetworkBuilder.createPlacesAndPersonDailySchedules();
